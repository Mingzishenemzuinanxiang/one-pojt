    ...
    <el-card shadow="always" :body-style="{ padding: '20px' }">
      <el-tabs v-model="activeName">
        <el-tab-pane :label="unread" name="first">
          <template>
            <el-table :data="unreadData" @tab-click="handclick">
              <el-table-column prop="title"> </el-table-column>
              <el-table-column prop="date"> </el-table-column>
              <el-table-column prop="id"> </el-table-column>
              <el-table-column prop="status">
                <template slot-scope="{ row }">
                  <el-button v-if="row.status === 1" @click="dian(row)"
                    >未读</el-button
                  >
                  <span v-else>已读</span>
                </template>
        ...
        <el-tab-pane :label="recycle" name="third"
          ><template> 回收站 </template></el-tab-pane
        >
      </el-tabs>
    </el-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    infonum() {
      ...
      let unnum1 = this.unreadData.filter((item)=>{
          return item.is_del===1
      }).length
      this.unread=`未读消息(${unnum})` 
      this.read=`已读消息(${this.unreadData.length - unnum})` 
