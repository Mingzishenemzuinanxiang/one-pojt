<template>
  <div>
    ...
    >
     ...
     <download-excel
        :data = "tabalLists.slice((currentPage - 1) * pagesize, currentPage * pagesize)"
        name = "data.xls"
        :fields="fileds"
        type="xls"
        >
    <el-button @click="dow" type="info">下载xls</el-button>
    </download-excel>
    <download-excel
        :data = "tabalLists.slice((currentPage - 1) * pagesize, currentPage * pagesize)"
        name = "data.csv"
        type="csv"
        >
      <el-button @click="dow" type="primary">下载csv</el-button>
    </download-excel>
...
export default {
  ...
  methods: {
    ...
    deleteRow(row) {
      this.tabalLists.splice(
        this.tabalLists.findIndex((item) => item === row),
        1
      );
    },
    save() {
